...
public class LayoutMachineTests : MiyamasuTestRunner
{
    ...
    public IEnumerator LayoutHTMLWithSmallTextHasValidView()
    {
        ...
        TagTree tree = null;
        yield return CreateTagTree(sample, tagTreeSource => { tree = tagTreeSource; });
        True(tree.viewHeight == 112, "not match. tree.viewHeight:" + tree.viewHeight);
    }
    ...
    public IEnumerator LayoutHTMLWithCustomTagMultipleInBody()
    {
        ...
        True(tree.GetChildren()[0].GetChildren()[1].offsetY.ToString() == "60.8", "not match of 2. actual:" + tree.GetChildren()[0].GetChildren()[1].offsetY);
    }
