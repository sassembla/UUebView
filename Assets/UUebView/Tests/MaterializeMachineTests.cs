...
public class MaterializeMachineTests : MiyamasuTestRunner
{
    ...
    public IEnumerator Culling()
    {
        ...
        // ここで、取得できたツリーに対して、MaterializeMachineを生成して扱う。
        var materializeMachine = new MaterializeMachine(core.resLoader);

        var materializeDone = false;

        var cor = materializeMachine.Materialize(
            rootObj,
            core,
            tree,
            new Vector2(tree.viewWidth, tree.viewHeight / 2),// これで50%強のコンテンツが出る
            0,
            () =>
            {
                materializeDone = true;
        ...
        // で、場に存在しているパーツから、表示されると不味いものを集める。コンテンツ内のゲームオブジェクトを集めて、その高さを見ればいいか。
    }

    [MTest]
    public IEnumerator CullingHalf()
    {
        var sample = @"
        <body>something</body><br>
        <body>something2</body><br>
    ...
    [MTest]
    public IEnumerator CullingHalf5()
    {
        var sample = @"
        <body>something</body><br>
        <body>something2</body><br>
    ...
    [MTest]
    public IEnumerator CullingHalf6()
    {
        var sample = @"
        <body>something</body><br>
        <body>something2</body><br>
